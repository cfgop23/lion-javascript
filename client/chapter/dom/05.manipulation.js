...
const template = `<div class='box'>${10 + 30}</div>`;

h1.insertAdjacentHTML('afterend', template);

//

// const listContainer = getNode('.listContainer');

const DATA = ['빨래하기', '게임하기', '유튜브 보기', '산책하기'];

const todoList = DATA.map((el) => `<li>${el}</li>`);

// todoList.forEach((el) => listContainer.insertAdjacentHTML('beforeend', el));
todoList.forEach((el) => insertLast('.listContainer', el));
